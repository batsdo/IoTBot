...
void Joy_Extended::joyCallback(const sensor_msgs::msg::Joy::SharedPtr joyMsg){
    ...
    std::copy(std::begin(joyMsg->buttons), std::end(joyMsg->buttons), std::begin(joyMsg_.buttons));
    std::copy(std::begin(joyMsg->axes), std::end(joyMsg->axes), std::begin(joyMsg_.axes));
// Verhindert das der Bot nach oder hinten fährt, wenn Objekt zu nahe 
    ...
    // Publishen der neuen veränderten Joy Werte
...
int main(int argc, char * argv[]){
    ...
    rclcpp::spin(std::make_shared<iotbot::Joy_Extended>()); //Starten der Node
    rclcpp::shutdown();                                     //Beenden der Node
